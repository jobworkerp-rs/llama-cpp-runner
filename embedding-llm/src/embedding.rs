...
use crate::sliding_window::{SlidingWindowProcessor, SlidingWindowConfig, MergeStrategy};
use crate::protobuf::embedding_llm::{EmbeddingLlmRunnerSettings, DType};
...
impl LlamaCppEmbedder {
    ...
    pub fn new_from_settings(settings: &EmbeddingLlmRunnerSettings) -> Result<Self> {
        ...
        let model_path = if settings.model_files.is_empty() {
            return Err(EmbeddingLlmError::configuration("No model files specified for llama.cpp".to_string()));
        } else {
            // XXX 複数ファイルの場合は最初のファイルを使用（llama.cppは単一ファイル）
            // (本当は複数ファイルのこともあるがEmbedding用の場合に分割されるほど大きなモデルは無い想定)
            if settings.model_files.len() > 1 {
                error!("Multiple model files specified, using first file: {}", settings.model_files[0]);
            }
        ...
        // dtypeのデフォルト値設定
        let dtype_enum = settings.dtype
            .and_then(|d| DType::try_from(d).ok())
            .unwrap_or(if settings.use_cpu { DType::F32 } else { DType::F16 });
        ...
        let tokenization_processor = Arc::new(if let Some(tokenizer_id) = &settings.tokenizer_model_id {
            ...
            TokenizationProcessor::new_from_model_id(
                tokenizer_id,
                settings.max_seq_length as usize,
            ).map_err(|e| EmbeddingLlmError::tokenization(format!(
        ...
        let sliding_window_config = settings.sliding_window_config
            .as_ref()
    ...
    fn l2_normalize(&self, embedding: &[f32]) -> Result<Vec<f32>> {
        ...
        if norm_squared == 0.0 {
            return Err(EmbeddingLlmError::inference("Cannot normalize zero vector".to_string()));
        }
        ...
        debug!("Normalized embedding, L2 norm: {:.6}", 
    ...
    pub fn health_check(&self) -> Result<()> {
        // モデルアクセス確認（スコープを分離してデッドロック回避）
        {
            ...
            if model.embedding_dimension() == 0 {
                return Err(EmbeddingLlmError::inference("Invalid embedding dimension".to_string()));
            }
            
            if model.max_context_length() == 0 {
                return Err(EmbeddingLlmError::inference("Invalid context length".to_string()));
            }
        } // モデルのロックを解放
...
/// テスト用ヘルパー関数
#[cfg(test)]
pub fn create_test_embedder() -> Result<LlamaCppEmbedder> {
    use crate::protobuf::embedding_llm::*;
    
    let settings = EmbeddingLlmRunnerSettings {
        model_id: "test-models".to_string(),
        use_cpu: true,
        dtype: Some(DType::F32 as i32),
        max_seq_length: 512,
        model_type: ModelType::Gguf as i32,
        model_files: vec!["Qwen3-Embedding-4B-Q4_K_M.gguf".to_string()],
        tokenizer_model_id: Some("Qwen/Qwen3-Embedding-4B".to_string()), // Qwen3-Embedding元モデルのtokenizer
        max_batch_size: Some(8), // テスト用バッチサイズ
    ...
    LlamaCppEmbedder::new_from_settings(&settings)
        .map_err(|e| EmbeddingLlmError::model_loading(format!("Test embedder creation failed: {}", e)))
}
