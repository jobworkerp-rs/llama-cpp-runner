...
impl SlidingWindowProcessor {
    ...
    pub fn new(
        ...
        if config.window_stride > max_seq_length {
            warn!("Window stride {} is larger than max_seq_length {}, using max_seq_length", 
    ...
    pub fn tokenize_text_and_instruction(
        ...
        let instruction_tokens = if let Some(inst) = instruction {
            let instruction_tokenized = self.tokenizer_processor
    ...
    pub fn merge_embeddings_static(
        ...
        if embeddings.is_empty() {
            return Err(EmbeddingLlmError::sliding_window("No embeddings to merge".to_string()));
        }
    ...
    pub fn merge_by_weighted_average_static(embeddings: &[Vec<f32>]) -> Result<Vec<f32>> {
        ...
        debug!("Merged {} embeddings by weighted averaging", embeddings.len());
        Ok(merged)
    }
...
mod tests {
    ...
    fn test_merge_embeddings_static() {
        ...
        // Test average merge
        let result_avg = SlidingWindowProcessor::merge_embeddings_static(&embeddings, MergeStrategy::Average);
        ...
        let result_weighted = SlidingWindowProcessor::merge_embeddings_static(&embeddings, MergeStrategy::WeightedAverage);
        ...
        let result_first = SlidingWindowProcessor::merge_embeddings_static(&embeddings, MergeStrategy::FirstWindow);
        ...
        let result_last = SlidingWindowProcessor::merge_embeddings_static(&embeddings, MergeStrategy::LastWindow);
        ...
        let result_single = SlidingWindowProcessor::merge_embeddings_static(&single_embedding, MergeStrategy::Average);
        assert!(result_single.is_ok());
        ...
        let result_empty = SlidingWindowProcessor::merge_embeddings_static(&empty_embeddings, MergeStrategy::Average);
        ...
        let result_mismatch = SlidingWindowProcessor::merge_embeddings_static(&mismatched_embeddings, MergeStrategy::Average);
