...
impl TokenizationProcessor {
    ...
    pub fn new_from_llama_model(
        ...
        let (eos_token_id, bos_token_id, model_path) = {
            let model_lock = model.lock().map_err(|_| {
                EmbeddingLlmError::tokenization("Failed to acquire model lock".to_string())
            })?;
            (
    ...
    pub fn tokenize_with_instruction(
        ...
        if let Some(tokenizer) = &self.fallback_tokenizer {
            let encoding = tokenizer.encode(full_text, false).map_err(|e| {
                EmbeddingLlmError::tokenization(format!("Tokenization failed: {e}"))
            })?;
            ...
            // 文字位置情報を取得
            // tokenizersクレートのget_offsetsメソッドを使用
            let char_positions: Option<Vec<(usize, usize)>> = {
        ...
        // Priority 2: llama.cpp built-in tokenizer (fallback)
            ...
            debug!(
                "Tokenized to {} tokens using llama.cpp built-in tokenizer",
                final_tokens.len()
            );
            ...
            Ok(TokenizedText {
                ...
                original_length,
                truncated,
                char_positions: None, // llama.cpp tokenizer doesn't provide char positions
            })
        } else {
            Err(EmbeddingLlmError::tokenization(
                "No tokenizer available".to_string(),
            ))
        }
    }
    ...
    pub fn tokenize_batch_with_padding(
        ...
        debug!(
            "Tokenized batch: {} sequences, max_length: {}",
            batch_tokens.len(),
            max_len
        );
    ...
    pub fn decode_tokens(&self, token_ids: &[u32]) -> Result<String> {
        ...
        // Priority 2: llama.cpp built-in tokenizer (fallback)
            ...
            Err(EmbeddingLlmError::tokenization(
                "No tokenizer available for decoding".to_string(),
            ))
        }
    }
...
pub struct TokenizedText {
    ...
    pub truncated: bool,
    /// Character positions for each token (start, end)
    pub char_positions: Option<Vec<(usize, usize)>>,
}
