...
mod tests {
    ...
    fn test_is_recoverable_llamacpp_errors() {
        ...
        for error in non_recoverable_errors {
            assert!(!error.is_recoverable(), "Error should be non-recoverable: {}", error);
            assert!(error.retry_delay_ms().is_none(), "Non-recoverable error should not have retry delay");
        }
        ...
        for error in recoverable_errors {
            ...
            assert!(error.retry_delay_ms().is_some(), "Recoverable error should have retry delay");
        }
    }
    ...
    fn test_always_non_recoverable_errors() {
        ...
        for error in non_recoverable_types {
            assert!(!error.is_recoverable(), "Error should always be non-recoverable: {}", error);
            assert!(error.retry_delay_ms().is_none(), "Non-recoverable error should not have retry delay");
        }
        ...
        assert!(io_error.is_recoverable(), "I/O errors should be recoverable");
    }

    #[test]
    fn test_always_recoverable_errors() {
        let recoverable_types = vec![
            EmbeddingLlmError::Io(std::io::Error::new(std::io::ErrorKind::TimedOut, "Connection timeout")),
        ...
        for error in recoverable_types {
            assert!(error.is_recoverable(), "Error should always be recoverable: {}", error);
            assert!(error.retry_delay_ms().is_some(), "Recoverable error should have retry delay");
        }
    }
}