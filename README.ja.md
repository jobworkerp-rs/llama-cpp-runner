...
- `modules/llama-cpp-rs` は `llama-cpp-rs` `0.1.151` ベースです。`0.1.151` で MTP speculative decoding の低レベル wrapper は利用可能ですが、この PR では互換追従のみを行い、`llama-cpp-plugin` の runner 設定・protobuf・生成ループにはまだ公開していません。公開対応は [Gitea issue #30](https://gitea.sutr.app/jobworkerp-rs/llama-cpp-runner/issues/30) で追跡します。
